TSICTestHarness.register({
    name: 'Settings: renders catalog groups',
            ...
            Json: JSON.stringify({
                Groups: [{
                    ...
                    Settings: [
                        ...
                        { Key: 'preset', Label: 'Preset', Type: 'enum', Options: ['Low','Med','High'], Value: 'Med' },
                    ],
                }],
            }),
        });
        await ctx.waitFor(() => ctx.doc.querySelectorAll('.group').length >= 1);
        ctx.expect(ctx.assert.domText(ctx.doc, '.group h3', 'Graphics'));
...
TSICTestHarness.register({
    ...
    async run(ctx) {
        ctx.inject('tsic.msg.UI.Settings.Catalog', {
            Json: JSON.stringify({ Groups: [{ Title: 'Audio',
...
TSICTestHarness.register({
    ...
    async run(ctx) {
        ctx.inject('tsic.msg.UI.Settings.Catalog', {
            Json: JSON.stringify({ Groups: [{ Title: 'Controls',
                Settings: [{ Key: 'jump', Label: 'Jump', Type: 'keybind', Value: 'Space' }] }] }),
        });
