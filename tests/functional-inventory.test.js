...
TSICTestHarness.register({
    name: 'Inventory: empty payload renders 32 empty slots without errors',
    file: '/screens/inventory.html',
    async run(ctx) {
        ctx.inject('tsic.msg.UI.Inventory.Updated', { OwnerId: 'Player', Items: [], MaxSlots: 32, MaxWeight: 50, CurrentWeight: 0 });
        await ctx.waitFor(() => ctx.doc.querySelectorAll('#inv-grid .tsic-slot').length === 32);
...
TSICTestHarness.register({
    ...
    async run(ctx) {
        ...
        ctx.expect(ctx.assert.eq(ctx.doc.querySelectorAll('#inv-grid img').length, 0));
    },
});
...
TSICTestHarness.register({
    name: 'Inventory: clicking a slot publishes UI.Cmd.Inventory.Use',
    file: '/screens/inventory.html',
    async run(ctx) {
        ...
        await ctx.waitFor(() => ctx.doc.querySelector('#inv-grid .tsic-slot[data-slot="3"] img'));
        ctx.clearPublishes();
        ctx.doc.querySelector('#inv-grid .tsic-slot[data-slot="3"]').click();
        ctx.expect(ctx.assert.published(ctx.handle, 'UI.Cmd.Inventory.Use', { where: p => p.SlotIndex === 3 }));
    },
});
...
TSICTestHarness.register({
    ...
    async run(ctx) {
        ...
        ctx.expect(ctx.assert.eq(ctx.doc.querySelector('#inv-grid .tsic-slot[data-slot="1"] img'), null));
    },
});
...
TSICTestHarness.register({
    name: 'Inventory: stack > 1 RMB opens drop quantity modal',
    file: '/screens/inventory.html',
    async run(ctx) {
        ...
        await new Promise(r => setTimeout(r, 30));
        // A range slider should appear in the document body.
